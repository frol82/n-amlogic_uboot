	...
	if (remote_mouse_event(dev, scancode, type, key_pointer_switch)) {
		...
		case 2:
			...
			input_dbg("scancode = 0x%04x, maptable = %d,code:0x%08x\n\n",
		         ...
		         //DTS 20160314 add remote LED blink 
                       	gp_remote->led_status_on ^= 0x1;
                        gpiod_direction_output(gp_remote->led_pin_desc,
                        gp_remote->led_status_on);
         		//
...
static void remote_release_timer_sr(unsigned long data)
{
	...
	//DTS 20160226 add remote led 
				{
				   input_dbg("xxxxxxxxxxxxxxxxx  custom code is xxxxxxx\n");
				   gp_remote->led_status_on=0x1;
     				gpiod_direction_output(gp_remote->led_pin_desc,
     			                                        gp_remote->led_status_on);
				}
...
static int remote_probe(struct platform_device *pdev)
{
	...
	remote->led_status_on, desc_to_gpio(remote->led_pin_desc));
        gpiod_direction_output(gp_remote->led_pin_desc,!gp_remote->led_status_on);
	//DTS
	...
	//DTS 
	gp_remote->irq = NEC_REMOTE_IRQ_NO;
